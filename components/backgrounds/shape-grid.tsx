...
export default function ShapeGrid({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      gradient.addColorStop(0, "rgba(7,7,11,0)");
      gradient.addColorStop(1, "rgba(7,7,11,0.85)");
      ctx.fillStyle = gradient;
      ctx.fillRect(0, 0, canvas.width, canvas.height);
    };

    const updateAnimation = () => {
      const effectiveSpeed = prefersReducedMotion ? 0 : Math.max(speed, 0.1);
  ...
  return (
    <canvas
      ref={canvasRef}
      className="pointer-events-auto h-full w-full border-none"
      aria-hidden="true"
    />
  );
}
